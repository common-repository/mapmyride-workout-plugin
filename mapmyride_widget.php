...
Plugin URI: http://wordpress.org/extend/plugins/mapmyride-workout-plugin/
Description: MapMyRideWidget used to display workouts added at MapMyRide, MapMyRun, MapMyFitness, etc.
Author: Rob Di Marco
Version: 1.0.2
Author URI: http://www.innovationontherun.com/
*/
define(MAP_MY_RIDE_PLUGIN_FILE, dirname(__FILE__) . '/mapmyride_widget.php');
  ...
  <p>
    ...
    <p style="font-size:90%">This URL can be found by clicking on the <a href="http://www.mapmyride.com/training_data">Data Center</a> at <a href="http://www.mapmyride.com">MapMyRide.com</a> and looking for the link <span style="font-weight:bold">RSS Feed for your Workouts</span></p>
  	...
  	<p>
		...
		<label for="mapmyride-do-limit"><?php _e('Would you like to only show certain elements of your workouts?')?>
		  <input type="checkbox" name="mapmyride-do-limit" id="mapmyride-do-limit" value="Y" <?= $options['do-limit'] == 'Y' ? 'checked="checked"' : ""?>/>
		</label>
		<label for="mapmyride-limit-field">
		  <select name="mapmyride-limit-field" id="mapmyride-limit-field" multiple="true">
		    <option>Date</option>
 ...
 */
function mapmyride_embed_callback($content)
{
    return preg_replace_callback("#[<\[]mapmy(?:ride|run|fitness|hike)[\]]*\]#i", 'mapmyride_embed_handler', $content);
}
function mapmyride_embed_handler()
{                
  ob_start();
  mapmyride(get_rss_url(), get_option("widget_mapmyride"));
  return ob_get_clean();
}                                                       
function mapmyride_options_menu(){
    if (function_exists('add_menu_page'))
    {
      add_options_page('Map My Ride', 'Map My Ride', 8, __FILE__, 'mapmyride_options');
    }
}
function mapmyride_options() {
    $basename = basename(MAP_MY_RIDE_PLUGIN_FILE);
    $message = "";    
    ob_start();
    ?>
    <style type="text/css">
    ...
    <div class="wrap">
      <form method="POST" action="<?= str_replace( '%7E', '~', $_SERVER['REQUEST_URI']) ?>">
        <? widget_mapmyride_control() ?>
        <input type="submit" value="Update Map My Ride Details" />
      </form>
    </div>
    <?php
    ob_end_flush(); 
}
add_action('admin_menu', 'mapmyride_options_menu');                                   
add_action("plugins_loaded", "mapmyride_init");
add_filter('the_content', 'mapmyride_embed_callback');
?>
